...
void get_boat_race_info(struct boat_race *races, char *time_info, char *distance_info) 
{
    ...
    for (int i=0; distance_info[i] != '\n'; i++) {
        if (isdigit(time_info[i])) {
            time *= 10;
            time += time_info[i] - '0';
            if (time_info[i+1] == ' ' || distance_info[i+1] == '\n') {
        ...
        if (isdigit(distance_info[i])) {
            ...
            if (distance_info[i+1] == ' ' || distance_info[i+1] == '\n') {
...
int main(void)
{
    ...
    for (int i=0; i < NUM_RACES; i++) {
        float d = (races[i].time * races[i].time) - (4 * 1 * (races[i].distance + 1));
        int low_num = ceil((-races[i].time + sqrt(d)) / -2);
        int high_num = floor((-races[i].time - sqrt(d)) / -2);
        printf("Low num: %d\n", low_num);
        printf("High num: %d\n", high_num);
